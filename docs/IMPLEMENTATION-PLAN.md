...
| Tests | 395 tests covering parsers/provenance, extraction, scenarios, deterministic command and phase resolution, difficulty bands, AI strategic state/family dispatch/hire ranking/equipment, combat, saves, replays, installers, and repository policy | Original-reference fixtures, checked-in save migrations, visual tests, larger-player AI stress, and native interactive installer/play tests |
...
| M6 | Foundation started | Deterministic objective-aware planner and replay driver; recovered outer planner, all family dispatch values, exact ten-scenario by seven-strategic-mode dispatch table, exact three-offer role rankings, action history, selector inventory and strategic routing including mode-6 leader/hostility movement, pair-flag Control, and family-11 equipment/cooldown, local Attack, mode-10 anchor and mode-16 follower paths; the family table, hire selector, and exact family-11 weapon choice are implemented as isolated rules pending strategic-role/planning-record integration; exact pre-city six-player reactions, directional attitudes, non-Homicidal recovery, combat/Control and sector Combat + Defense advantage hostility, hostility-aware targets, hashes/saves/replays; exact 0/1/2 per-computer resolution calibration implemented for all nine consumers | Recover strategic-role transitions and integrate exact hiring/planning records before replacing provisional scalar scoring; capture reference boundaries and larger-player/objective-completion stress cases; M5 gate |
...
Current automated baseline: the solution builds successfully, 395 tests
