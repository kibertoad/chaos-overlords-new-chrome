...
- The current implementation baseline includes the recovered sector Combat +
  ...
  dispatch, exact isolated hire-offer ranking/rejection and 395 passing tests. Native saves remain
...
| M6 | Foundation started | Deterministic objective-aware planner and replay driver; recovered outer planner, all family dispatch values, exact ten-scenario by seven-hire-role dispatch table, exact base turn schedules and three-offer role rankings, action history, selector inventory and strategic routing including mode-6 leader/hostility movement, pair-flag Control, and family-11 equipment/cooldown, local Attack, mode-10 anchor and mode-16 follower paths; the family table, base hire schedule, Power/Kill 'Em All/Big 40, Eliminate, Siege, Big Man, and Armageddon adjustments, hire selector, and exact family-11 weapon choice are implemented as isolated rules pending the three remaining adjustment branches/planning-record integration; exact pre-city six-player reactions, directional attitudes, non-Homicidal recovery, combat/Control and sector Combat + Defense advantage hostility, hostility-aware targets, hashes/saves/replays; exact 0/1/2 per-computer resolution calibration implemented for all nine consumers | Recover Greed/Acceptance/Dominance hire-role adjustments and integrate exact hiring/planning records before replacing provisional scalar scoring; capture reference boundaries and larger-player/objective-completion stress cases; M5 gate |
...
Current automated baseline: the solution builds successfully, 453 tests
