...
- [1. Definition of complete](#1-definition-of-complete)
  - [Explicit non-goal: original network code and protocols](#explicit-non-goal-original-network-code-and-protocols)
- [2. Current baseline](#2-current-baseline)
  - [Handover checkpoint - 2026-09-11](#handover-checkpoint---2026-09-11)
- [3. Engineering principles](#3-engineering-principles)
  - [3.1 Manual-derived implementation checklist](#31-manual-derived-implementation-checklist)
- [4. Required technical documentation](#4-required-technical-documentation)
  - [4.1 Documents to maintain](#41-documents-to-maintain)
  - [4.2 Evidence rules](#42-evidence-rules)
  - [4.3 Original internal-structure research](#43-original-internal-structure-research)
  - [4.4 Full accuracy audit](#44-full-accuracy-audit)
- [5. Workstreams](#5-workstreams)
  - [A. Evidence and validation harness](#a-evidence-and-validation-harness)
  - [B. Asset acquisition and catalog](#b-asset-acquisition-and-catalog)
  - [C. Core model and deterministic runtime](#c-core-model-and-deterministic-runtime)
  - [D. New-game setup and city generation](#d-new-game-setup-and-city-generation)
  - [E. Turn structure, economy, and world state](#e-turn-structure-economy-and-world-state)
  - [F. Gang lifecycle and commands](#f-gang-lifecycle-and-commands)
  - [G. Items, equipment, and research](#g-items-equipment-and-research)
  - [H. Combat and other opposed systems](#h-combat-and-other-opposed-systems)
  - [I. AI](#i-ai)
  - [J. User interface and input](#j-user-interface-and-input)
  - [K. Audio and video](#k-audio-and-video)
  - [L. Persistence and compatibility](#l-persistence-and-compatibility)
  - [M. Hot-seat play](#m-hot-seat-play)
  - [N. Platform, packaging, and quality](#n-platform-packaging-and-quality)
- [6. Milestone order and dependency gates](#6-milestone-order-and-dependency-gates)
  - [Current milestone status (2026-09-08)](#current-milestone-status-2026-09-08)
  - [M0 - Research foundation](#m0---research-foundation)
  - [M1 - Faithful city setup and UI shell](#m1---faithful-city-setup-and-ui-shell)
  - [M2 - Complete command model](#m2---complete-command-model)
  - [M3 - Economy, sites, research, and equipment](#m3---economy-sites-research-and-equipment)
  - [M4 - Combat and full turn resolution](#m4---combat-and-full-turn-resolution)
  - [M5 - Objectives and full hot-seat game](#m5---objectives-and-full-hot-seat-game)
  - [M6 - AI parity](#m6---ai-parity)
  - [M7 - Audiovisual parity](#m7---audiovisual-parity)
  - [M8 - Compatibility and cross-platform release](#m8---compatibility-and-cross-platform-release)
- [7. Cross-cutting test matrix](#7-cross-cutting-test-matrix)
- [8. Completion tracking conventions](#8-completion-tracking-conventions)
- [9. Immediate next implementation sequence](#9-immediate-next-implementation-sequence)
...
|---|---|---|
| Build | .NET 10 solution, MonoGame DesktopGL 3.8.5.1, xUnit v3 4.0.0; self-contained Windows/Inno, Linux/Debian, and macOS arm64/x64/pkg installers; manual-only release and cross-platform validation workflows; pull-request/manual zizmor gate; Windows directory deployment with native-platform smoke test, visible/retryable legal-asset import, and startup-error reporting; green cross-platform installer run `34403047147` and zizmor run `34403047115` | macOS signing/notarization and native interactive play tests |
| Original data | Embedded 22 sites, 90 gangs, and 64 items with pinned provenance; deterministic format-1 regeneration command and shared semantic schema validation for record counts/order, names, special sites, item categories/media, and unused sentinels | Formula-use validation and unresolved semantic field corroboration |
| Extraction | Transactional/versioned exact-inventory and full-pack SHA-256 validation with stale-output replacement and schema-v1 machine-readable diagnostic reports; 215 repaired RGB555 PX16 images, 214 retained plus decoded PX08 resources, 28 WAVs, 8 Ogg tracks, 2 bounded Smacker-v2 videos with exact timing and complete managed palette/audio/indexed-frame validation, bounded legacy WinHelp text/topic/context/font/hotspot decoding, help and opaque files; generated 686-output factual catalog; native loader, palette, opaque-copy/stretch, pattern, and exact-white key paths recovered | Native pixel comparison at the 16-bit white-key/display-driver boundary, exact WinHelp typography/paragraph geometry, semantic role/owner resolution |
| Simulation | Deterministic city seed with recovered once-per-process low-16-bit uptime-clock local derivation, six players, stable gang IDs, typed command queue, headless phase coordinator, Upkeep, all 14 command resolvers, binary player/roster-slot Instant, Combat/police, Chaos, and Movement scans plus submission-ordered cash transactions with exact simultaneous per-player Move-capacity normalization, recovered pre-Combat Chaos preparation/post-Transaction payout split, deferred gifts and Sell payout overwrite, simultaneous gang/Crackdown combat with exact police visible/Hide detection and Force 5 + Combat 20 attack pools, same-turn newly created police attack and duration tick, exact one-off/recurring Hide lifecycle, additive 3–5-turn police duration and exact inclusive-five-turn two-slot control-loss history, hidden attack/visibility checks, and local influenced-site stats | Crackdown notification fixtures, transaction runtime corroboration, and native RNG-stream correlation using captured uptime seed plus `serialNum` state |
| Client | Scaled 640x460 routed setup/handoff/city/sector/gang/finance/ranking/items/Give/Sell/combat-summary/search/commands/hire/events/endgame/help UI backed by authoritative `MatchState`; distinct whole-city and detailed-sector projections composite original ownership tiles, the latter as a clickable 3x3 neighborhood beside all three building portraits; original `PX05013` Sell and `PX05015` Give independently toggle any equipped slots and confirm one exact multi-item transaction, with Give then choosing one compatible same-sector recipient; normal planning turns auto-resolve internal phases while debug mode can step them; persistent original-art Hire dock supports portrait drag/drop from city or sector detail, split price/reject footers and `HIRED` stamping; role-specific `PX00129` copies preserve opaque pixels and exact-white-keyed gang/status/drag overlays; local setup preserves the selected-card gate, opaque 32-by-30 portrait scaling, and keyed `PX00140` selected arrows without borrowing the separate `PX00146` stencil; `PX05010` automatically pages queued turn reports after handoff; `PX05012` presents sector-paged combat results; F1 presents locally decoded original help with exact contextual entry, styled text, 67 clickable topic jumps, 26 modal popups, and keyboard/mouse paging; recovered item-selected eight-frame attack/hit, retaliation, evasion and police animation playback; recovered setup with exact local player-card click/drag geometry; exact main-console tiles, horizontal subroutes, and pressed/release behavior; local controls; private handoff; Core-derived commands, projections, visibility, combat results, research/equipment transfer and notifications; mouse/keyboard, saves/replays; restart-safe online reconstruction from verified snapshots, sealed turns, and current whole-document drafts; recovered title/setup, gameplay, and endgame CD soundtrack programs with exact static menu ownership/restart boundaries plus repeat and focus pause/resume; managed startup logo/intro streaming with indexed rendering, decoded PCM, deterministic cadence, explicit skip, focus pause, and safe failure; persisted base/current statistics, automatic Detailed/Simple combat, bounded foreground-only panel motion, independent 0-10 audio, visible sector-modal idle-gang warning, planning-timer, and recreation-native windowed/borderless-fullscreen choices; global F11 and Alt+Enter display switching preserve the 640x460 virtual coordinate system; legacy color depth explicitly always on | Remaining sprites/atlas and management panels/hit maps, exact WinHelp typography/paragraph geometry, remaining sound-effect triggers, exact Options cadence, original startup trigger/skip capture and native A/V validation, accessibility |
| Tests | 1575 tests covering parsers/provenance, exact manifest-to-disk asset inventory and schema-v1 JSON verification diagnostics, bounded WinHelp decompression/topic/context/font/hotspot extraction, native context hashing, styled wrapping, exact link/popup resolution, contents-backed and exact-symbol navigation, in-context executable formula augmentation with named listed missing-subject fallback, viewer data/layout, bounded and privacy-filtered runtime diagnostics/export, extraction, scenarios and Siege landmarks, original low-16-bit uptime-clock seed derivation and nonpositive inclusive-bound clamping, deterministic command and phase resolution including binary $3 direct-tolerance Bribe behavior, binary-ordered cumulative per-gang Influence and same-item Research completion suppression, player/roster Combat and police RNG streams with full overkill-stat credit plus exact police detection/attack boundary curves, Terminate-before-Move and roster-capacity scheduling, Instant effective-stat and board-ordered Control tie-breaks, cross-player Control owner/defense snapshots, binary-recovered tie selection, retained empty-sector ownership, exact Crackdown history boundaries, split pre-Combat Chaos preparation/post-Transaction payout scheduling, same-turn newly created police attacks/duration tick, prepared-Chaos save continuation, fixed-slot Chaos RNG ordering, exact one-off/recurring Hide retention, replacement and counting, one-to-three-item Give/Sell validation/resolution/persistence, submission-ordered cash transactions, deferred Give replacement, same-slot Give swaps, original multi-Sell payout overwrite, pre-existing Factory pricing, same-turn completion deferral, and matching last-slot-overwrite multi-item Sell finance projection, original command-panel layout/composition including Movement, Gangs in Sector, and Search: Sites with exact per-player filter/marker projection, Last Turn Events read-progress, last-completed-turn Combat filtering, bounded page navigation, and per-player presentation cursors, immediate single-player defeat, global sole-survivor completion, victory/elimination splash and Awards/Stats routing, recovered all-scenario ranking scores/ties/inactive order, bounded Comlink delivery/persistence/replay and client layout/composition, local-human setup/name/portrait/color-slot policies and sparse completion, setup inset-selection faces, top-bar active-player animation, Game Info and City/Sector Finance layout/projection, selected-sector Gang Information rosters/equipment cells, Player Ranking rails/ties/objective scores, pointer-button press edges, difficulty bands, AI strategic/planning state and complete action tuples, family dispatch and live boundary branches, sector selection and live-equivalent hire refills/placement, isolated Advanced policy behavior, defender safeguards, persistence and paired 12-seed playtesting, eliminated-player planning skips, negative-effective-Stealth police detection, original city/setup vectors, hire schedules/limits/ranking/equipment, combat, saves, replays, verified self-healing primary recovery and backup-only slot discovery, soundtrack/effect discovery, screen programs, volume policies/preferences including display-mode migration, idle-gang warning, planning-timer quantization/cadence/boundaries/state, online restart reconstruction and whole-draft restoration, installers, repository policy, versioned replay-operation, exact payload, and command-target gating, complete event/phase-history hash authentication plus event, notification, Comlink, and outcome structural validation including future-turn, event-boundary, and unknown-gang rejection, replay-verified 40-turn six-computer objective activity across seven guarded seeds, natural Big Man completion by turn 60, observable bounded long-run validation, and presentation-free deterministic headless-runner bounds, progress, policy, and replay checks, plus clean-boundary player-control transfer hashing, persistence, replay-version gating, and lockstep AI takeover, ordered live/reconnect takeover voting and approval, return cancellation, event-history gap rejection, sealed-event digest binding, and duplicate delivery | Original-reference fixtures, visual tests, tournament coverage approaching the release-scale matrix/objective completion, and native interactive installer/play tests |
...
The client exposes a default-off Advanced AI option through an isolated policy
layer over the Original planner. Original AI remains the parity target and
default; Advanced results must not be used as evidence for Original parity.
...
- The current implementation baseline includes the recovered sector Combat +
  ...
  12-16 sector-selection kernels, live recovered hire-placement rules, exact
  all-scenario standings, and global sole-survivor completion. The current gate
  passes 1,428 tests. Native saves are v22, replay is v24, and the canonical
  hash is v25.
...
- The original `PX05017`/`PX05018` Comlink panels now provide bounded incoming
  message paging, unread indication, multi-human recipient selection, and the
  recovered four-by-40-character editor over replay-recorded Core operations.
...
- Original local Begin now converts every omitted player slot to a computer,
  ...
  permanent Crackdown duration 100 only to sectors still neutral after all six
  HQs are owned.
- The remaining recovered fresh-name modifiers are authoritative: `SMGSPANK`
...
- Family 3's complete handler is live, including the cash-site continuations,
  ...
  Terminate override. Its action targets and RNG consumption are replay-wired.
- Family 5's complete Support-site counterpart is live, including mode 7's
  exclusion of sectors with an existing previous-Influence assignment, the
  shared opponent-pool asymmetry and combat comparison, and both terminal
  overrides. Its decisions and RNG consumption are replay-wired.
- Family 7's complete research-specialist handler is live, including its
  one-draw hostile Attack opportunity, equipment and Heal gates, strict owned-
  sector Research scoring, encoded movement, first-useful-site Influence,
  exact Research item continuation/fallback order, exhausted-research family-0
  transition, Greed override, and persisted polymorphic focus value.
- Family 9's complete handler is live, including weapon/armor priority without
  an existing-cooldown gate, cost-times-three cooldown replacement, owned-
  sector mode-3 movement, the five-draw Attack loop, previous-Control movement,
  and non-owned-sector Control. Exact targets and RNG are replay-wired.
- Family 10's complete stealth/concealment handler is live, including selector
  `0x72`'s strict-Defense armor choice, literal cooldown 2, the fixed Smoke
  Bombs opportunity, no-visible-opponent Heal gate, completed-Stealth mode 9,
  its intentional probe/redraw RNG order, and the prior-Chaos Hide fallback.
- Family 12's complete handler is live, including no-visible-opponent weapon,
  armor, miscellaneous, and Heal priority; raw-cost equipment cooldowns; the
  encoded-current-sector all-zero tie Move; the five-draw visible-opponent
  Attack loop and its target/comparison ordinal asymmetry; and Greed's final-
  three-turn Terminate override. Its commands and RNG consumption are replay-
  wired.
- Static kernels guard family-1's cash 50/51, Force 8/9, effective-Heal -3/-4,
  ...
  RNG, all-zero post-filter fallback, and six-gang routing capacity. The exact
  post-equipment continuation is live and tested, including its raw-owner-
  greater-than-zero asymmetry, both equipment cooldown shorts, selector
  `0x6c`'s nearby-danger gate and index-64 aliases, exact weapon-first/armor-
  second selection, and actual Equip resolution. Capture controlled original
  decisions for the remaining selector contexts, then follow with multi-seed
  tournament and objective-completion AI traces.
- Static readings of the computer players go into `FND-AI-*` findings and
  their behaviour into `RULE-AI-*` rules in `spec/`; implementation status
  belongs in `PARITY.md` and open static work in `static_validation_plan.md`.
  Never commit proprietary assets or decompiled original source text.
...
4. **Original and modern behavior are separate.** Compatibility is the default.
   ...
   belong exclusively to a named, default-off Advanced AI option. Fixes,
...
- Recreate Options for color depth, music, sound effects, base statistics,
  ...
  Advanced AI. Original AI is the default; Advanced AI is a recreation
...
- Recreate the remaining City View, Sector View, 9-sector display, Overlord Bar,
  Main Control Panel, Last Turn Events, Conflict/email, Gang Upkeep,
...
- Chaos uses generated sector Income + Force + Chaos for each gang, earns cash
  subject to sector control, aggregates successes only in resolver-local
...
- Crackdown police attack all gangs in the sector with Force 5 + Combat 20
  minus effective Defense dice at 5+. Executable detection is
  `clamp(115 - 5 * effective Stealth - (Hide ? 20 : 0), 0, 100)`, superseding
  the abbreviated manual table and its separate Police Detect 12 listing.
  Sector ownership and influenced-site effects must be validated.
...
- Each controlled sector grants $1 sector tax. The shipped updater folds this
  together with influenced-site Cash in one recomputed sector byte; the
  recreation presents the equivalent components separately. Site protection,
  city-official chaos estimates, and projected cash adjustment must be reproduced.
...
The original game is documented in `spec/` under version 1 of the
[documentation standard](https://dinorefurb.com/documentation-standard/); the
rebuild's departures are in `DEVIATIONS.md` and its progress against the spec
in `PARITY.md`. `node tools/check-spec.mjs` checks all three and regenerates
`spec/index/`. The tables of contents and the decision index of the documents
under `docs/` are rebuilt with `node tools/update-doc-indexes.mjs` (`--check`
reports stale blocks); `ASSET-CATALOG.md` is regenerated by the extractor's
`--catalog` mode.
...
|---|---|---|
| `docs/README.md` | Catalog of every document by purpose, and where each subsystem lives in the spec and the design documents | Every document is reachable from the index |
| `spec/` | Builds, sources, findings, experiments, formats with Kaitai definitions, rules in pseudocode, bugs and screens, each with a stable ID, a status and its evidence | Every consumed original byte, rule and screen has an entry, and the standard's checks pass |
| `PARITY.md` | One row per rule, format and screen entry: how much of it the rebuild does and which tests compare it with the original | Every eligible entry has a row and the counts match |
| `DEVIATIONS.md` | Every deliberate departure from the spec, with its reason and setting | Every intentional deviation is recorded |
...
| `AI-SPEC.md` | The rebuild's Original and Advanced AI policy architecture and planner invariants; the original's AI itself is in the `AI` area of the spec | Every original AI rule has a policy that implements it |
...
| `VALIDATION.md` | How to run experiments on the original, capture reference runs, write fixtures, compare state, and reproduce failures | Clean-room parity procedure works from a fresh checkout |
| `DECISIONS.md` | Dated product, security, library and format decisions that are not departures from the original | Every decision that shapes the product is recorded |

### 4.2 Evidence rules

Evidence follows the documentation standard. A static reading of the executable
or a data file is a finding with its addresses or offsets; a run of the
original is a dynamic finding or an experiment with a fixture. A rule, format
or screen cites them and takes the status they support: `unknown`, `sourced`,
`supported`, `established`, `disputed` or `superseded`. Findings and
experiments are `recorded`, `reproduced` or `superseded`. There is no separate
confidence scale.
...
Behavioral parity requires an experiment fixture from the original and a test
that replays it.
...
- Maintain exact-inventory `verify`, automatic stale-output replacement, and
  `--force` behavior; add a machine-readable `list` mode without overwriting a
...
- Maintain the bounded decoder that converts the user-owned `HELP/Chaos.hlp`
  and companion `CHAOS.CNT` into a locally generated, versioned 80-topic
  representation with readable text, titles, contents hierarchy, listed status,
  all 80 native context hashes/offsets, and the 59 verified contents symbols.
  The supported file has an empty numeric context map. Recover links and inline
  formatting without checking in or distributing the copyrighted help text.
...
formulas for each must be captured in `OBJECTIVE` rules in `spec/`; names alone are not
sufficient evidence.
...
A deterministic twin-resolution fixture drains Detailed playback and proves the
same rolls, Force, phase hash, and complete authoritative hash as Simple mode;
...
- Put beneficial but parity-breaking choices behind a persistent `Advanced AI`
...
Advanced AI has separate deterministic fixtures and is never used to make an
...
- Map every PX sheet and sub-rectangle in the `SCR-*` screen entries in `spec/`.
...
- Maintain the F1 Help route and cross-platform in-game viewer backed by the
  locally extracted help representation. The inert Help item in the GOG
  ...
  clear legal-copy/import error when help content is unavailable. Route current
  screen-based entry through the exact recovered `CHAOS.CNT` context symbols,
  preserve inline topic jumps, definition popups, and authored font emphasis.
  Append high-confidence executable formulas inside their matching original
  subjects at presentation load, resolving exact context symbols before title
  fallback. When a compatible pack lacks the relevant subject entirely, add a
  specifically named listed subject rather than a generic recovered-rules page.
  The cross-platform pixel viewer implements those semantics; exact native
  WinHelp typography and paragraph geometry remain a visual-capture task.
...
- Preserve the recovered streamed programs: Track 2 repeats for title/setup,
  ...
  than compatibility targets. The exhaustive selector-call inventory fixes the
  precise menu restart boundaries. Keep music independently adjustable. Missing
  or unreadable music must degrade silently to gameplay without music, never
  stall the turn loop.
- Preserve the implemented managed logo/intro stream, deterministic cadence,
  explicit skip controls, focus pause, and graceful failure when optional video
  is absent. Capture the original trigger/order/skip policy and validate native
  color/audio fidelity before classifying it as parity.
...
- Add autosave, atomic writes, backups, backup-only slot discovery, and validated
  self-healing recovery of missing or corrupt primary generations.
...
- Preserve delayed site completion across a mid-turn native save and across the
  following replayed planning boundary; both paths must activate the same site
  benefits and converge on the same canonical state hash.
...
- Retain the implemented bounded structured session logs and local crash
  reports plus the explicit privacy-filtered Options export. Keep raw exception
  messages and filesystem paths out of shareable bundles.
...
|---|---|---|---|
| M0 | In progress | Research templates; file/binary logs; architecture, validation and parity documents; source/output verification; rollback-safe staged installation; sanitized reference-fixture schema; labeled JSON state-diff tool; versioned 686-output manifest and generated factual asset catalog; deterministic format-1 gameplay-table generator with shared semantic validation | One captured, fully documented reference observation |
| M1 | Foundation started | Ten scenario definitions and durations; recovered one-to-six local-human setup with bounded names/portraits, face-drag color move/exchange, ascending sparse-slot Computer completion with unique portrait/name RNG, and atlas-aligned `PX00143` top-strip/player-card apertures, arrow hit areas, and name rows; 32x32 density-derived 8x8 city, balanced three-site rejection sampling, explicit sector income/tolerance, fixed-candidate HQ permutation, Right Hands Force 10, Armageddon exclusions/overrides, all six exact rule-changing `SMG*` names including extra-gang starts and global visibility; title/setup/city router with scenario/duration/local-player controls, original overlord portrait selection, global AI Mentality panel, manual-backed `PX05021` Game Info panel with multiplayer/load auto-open, virtual-coordinate mouse input, and mapped PX00130/PX00143/PX00128 frames | Original seed/setup fixture and golden screens |
| M2 | Foundation started | Centralized structural limits; explicit headless setup/player/sector/site/gang/research/inventory/hire/statistics schema; stable IDs; phase coordinator; common typed validation-rule pipeline; deferred hire purchase/placement, one-offer-per-turn snubbing, binary-derived 1–89 rejection refill and 5–9 initial Force; player elimination; declarative validation for all action target shapes; typed queue mutations and ordered events; bounded mechanical notifications plus the separate recovered 16-entry Comlink inbox; recovered RNG; canonical phase hashes | Runtime offer/Force fixtures, within-phase ordering, and remaining reference-derived edge rules |
| M3 | Foundation started | Ordered base Upkeep economy; distinct generated sector Income for Chaos/Control and owner-only tax-plus-site Cash for Upkeep/UI; non-mutating original City/Sector Financial projection; all Instant actions with binary-recovered player/roster-slot order and Transaction cash actions in player submission order, deferred Give application and Sell payout overwrite; separate Terminate-before-Move player/roster passes with retained last-gang sector ownership; cumulative per-gang Influence, same-item Research completion suppression, and board-ordered grouped Control; phase-opening effective-stat snapshots for simultaneous Instant rolls; one phase-opening owner/defense snapshot and binary-recovered player-slot tie selection with a leading neutral candidate at zero margin for cross-player Control conflicts, including execution-time Crackdown lockout; resolver-local per-turn Chaos with binary player/roster roll order, pre-Combat Crackdown creation, persisted prepared outcomes, and post-Transaction grouped payout; same-turn new-police combat/duration tick; permanent neutral-sector `SMGISLANDS` Crackdown; pre-planning activation of completed-site stats; manual-backed sector/site ownership invariants for all three special buildings; Research caps; binary-recovered Factory division/rounding with delayed acquisition/replacement coverage; recovered 0/1/2 difficulty pools and thresholds for Heal, Influence, Research, Chaos, plus high-band owned-sector Crackdown reduction | Complete binary police notification fixtures, transaction runtime corroboration, and the M2 gate |
| M4 | Foundation started | Recovered raw RNG/range algorithms; serializable state; effective item/site stats; phase-wide Attack/retaliation and police snapshots; recovered exact active/recurring Hide lifecycle, police visible/Hide detection and Force 5 + Combat 20 attack pools; recovered difficulty-specific d20 hidden detection, band-0 Defense reduction, 6+/5+/4+ main rolls with quarter-pool damage floor, exact Hide/weapon/Martial-Arts retaliation eligibility, 5+/5+/4+ halved retaliation, elimination and recorded rolls; Simple/Detailed twin-resolution hash equivalence plus cancellation and large-elapsed no-hang coverage; opening, retaliation, unarmed, evasion and detected-police animation/audio routing | Investigate the reported GOG/1.1 Detailed Combat freeze, complete reference combat/Crackdown notification fixtures, validate animation cadence/color keys, and M2-M3 gates |
| M5 | Foundation started | Scenario predicates, exact all-scenario scores and competition standings with player-slot tie order/trailing inactive players, binary-ordered end-turn evaluation and global sole-survivor completion; manual-backed Siege setup marks all six starting controlled HQ sectors important and the city renders the exact native white-keyed pylon pair on those landmarks and Big Man's four center sectors; Big Man accrual, Eliminate cleanup, state-derived winners, all five award projections, outcome events/notifications and hashing; recreation-native v22 snapshots and v24 authoritative-operation replays with bounded validation, complete immutable event-history, phase-boundary-history, and per-record Comlink-read authentication with append-cached canonical event bytes plus legacy shape/sequence validation, four-target multi-item command persistence, read-back-before-promotion, last-valid-generation backups, self-healing missing/corrupt primaries, backup-only slot discovery, client recovery status, and reusable migration machinery; complete one-to-three-item Give/Sell selection and resolution workflows | Special objective/tie fixtures, objective golden screens, additional persistence fuzz/soak coverage, remaining management UI deliverable and M4 gate |
| M6 | Foundation started | Deterministic objective-aware planner and replay driver; recovered outer planner, every family handler, exact ten-scenario by seven-hire-role dispatch table, exact base turn schedules and three-offer role rankings, complete three-generation action tuples, selector inventory and strategic routing including modes 2-3, 5-10, 12-16, and encoded family-6/7/12 movement, exact ten-scenario standing reconstruction, complete live family-0/4 general-purpose state machines, complete live family-2 aggressive territorial handler, complete live family-3/5 cash/Support Influence handlers, complete family-6 coverage/Move/equipment/Attack handler, complete family-7 Research handler, complete family-9 equipment/Move/Attack/Control handler, complete family-10 stealth/concealment handler, complete family-12 equipment/Heal/Move/Attack handler, complete live family-13/14 handlers with off-objective Move, owned-objective Heal/equipment/Influence, owned/contested exact Attack/Heal/Control, and family-14's prior-Control Heal/family-13 transition, mode-6 leader/hostility movement, pair-flag Control, family-1 equipment/cooldown and all three bounded continuations, and complete family-11 equipment/cooldown/Heal/local-Attack/mode-10 anchor/mode-16 follower paths; the family table, every objective-specific hire-role adjustment, computed gang limit, hire-attempt gate, live post-command role update, exact live hire selector and failure-path snub, fixed three-slot offer tombstones/refill, ascending inactive roster-slot reuse/reset, exact family-0/1/2/4/6/11 equipment choices, first-planning lifecycle, duplicate Chaos/Influence cleanup, and live persistent-anchor placement refresh with ordinary, Big Man, visible-hostile, and Siege paths are implemented; verified current/previous hire roles, first-plan flags, action/target tuples, equipment cooldowns, polymorphic focus/formation values, family-6 coverage sectors, and six-by-81 family slots persist in authoritative state, with exact role rollover and active-gang family assignment during planning preparation; exact pre-city six-player reactions, directional attitudes, non-Homicidal recovery, combat/Control and sector Combat + Defense advantage hostility, hostility-aware fallback targets, hashes/saves/replays; exact 0/1/2 per-computer resolution calibration implemented for all nine consumers; dedicated structure pass isolates dispatch, immutable planning facts, shared recovered operations, and provisional fallback scoring without changing traces; live-equivalent offer refills are included in replay-verified six-computer fixtures, all objectives must resolve hires, expand, and produce scenario-specific combat/control signals through turn 40 across seven guarded seeds, Big Man completes naturally by turn 60 at one guarded seed, eliminated planning slots are skipped through recorded transitions, negative effective Stealth is bounded safely in police detection, and long tournament runs expose scenario/seed plus ten-turn boundary/event/elapsed diagnostics under the bounded canonical runner; the reusable presentation-free tournament command adds bounded parallelism, heartbeats, deterministic seed matrices, per-match territory/defense/combat metrics, and configurable replay sampling for both AI policies | Capture fixed recovered-family reference boundaries, broader multi-seed tournaments, and evidence-led completion behavior for the other five objective scenarios; M5 gate |
...
non-ideal shipped behavior. Extend the separate, default-off Advanced AI option
...
targets. Advanced AI results do not count as original-parity evidence.
...
independent recovered 0-10 Music and Sound Effects scales and safely persists
both levels, base/current gang statistics, automatic Detailed/Simple combat,
bounded panel motion, the idle-gang warning, and the recovered four-choice
planning timer. Global F11 display switching preserves compatibility coordinates.
Right-button press cancellation spans transient interactions and nested panels.
The presentation toggles use the recovered Current-statistics,
Detailed-on, Slide-on, and idle-warning-on defaults. Legacy color depth is
...
close/adjacent-buffer/combat cadence, native validation, video, and exact
visual behavior are still required. Preference persistence and soundtrack
menu/program restart boundaries are statically closed.
...
authoritative unread state clears. Opening Comlink View and paging now acknowledge
only the displayed record in current save v24, replay v28, and canonical hash v27;
legacy saves and replays deterministically retain their earlier read-through meaning.
...
`PARITY.md` tracks completion in the standard's form. Each row's Code is
...
1. Expand headless deterministic Original AI tournaments and objective-completion
   stress cases for Kill 'Em All, Big 40, Eliminate, Siege, and Armageddon. The
   presentation-free `Rechaos.Tools ai-tournament` runner now provides bounded
   parallel workers, deterministic seed scheduling, heartbeats, per-turn traces,
   JSON results, and configurable replay sampling for release-scale and paired
   Original/Advanced samples.
   ...
   for every seed.
2. Harden the modern online path with automatic host-snapshot recovery where
   the protocol already permits it. Deterministic player-approved takeover is now
   complete: departures and wholly missed timed turns open a unanimous wait-or-AI vote;
   returning activity cancels a pending absence; replay v26 records the one-way,
   clean-boundary, hash-authenticated Human-to-Computer transfer only after approval;
   and reconnect validates and replays votes, transfers, and seals in exact order. Lobby browser,
   spectator, and chat work remain separate product choices rather than parity
   requirements.
...
11. Finish macOS signing/notarization and the complete release gate after the
    deferred parity and native-platform evidence is available.

## 10. Source hierarchy

The spec's source entries are `SRC-MANUAL-GOG`, `SRC-HELP-GOG` and
`SRC-RECHAOS-3561D41`. Only evidence from the original can dispute a spec
entry; a source that disagrees with the findings is recorded in the entry's
What the sources say or Open questions section. When sources disagree, use this
order and record the conflict:
