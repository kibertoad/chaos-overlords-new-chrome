...
| Client | Scaled 640x460 title/setup/handoff/city/commands/events/endgame router backed by authoritative `MatchState`; recovered setup; local setup controls; private ready handoff; Core-derived legal command picker; replay-recorded notification dismissal; mouse/keyboard, saves and replays | Full setup detail, richer target workflow, sprites/atlas, remaining management panels/hit maps, AI driver, animation, sound/music, accessibility |
