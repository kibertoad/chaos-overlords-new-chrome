...
Current automated baseline: the solution builds successfully, 1518 focused
tests pass in the default gate, all 1571 pass when the 53 long-running AI
campaign cases are explicitly included, and the inspected legal-copy output
...
2. Harden the modern online path with automatic host-snapshot recovery where
   the protocol already permits it. Deterministic departed-seat takeover is now
   complete: replay v26 records the one-way, clean-boundary, hash-authenticated
   Human-to-Computer transfer; the live session applies only authoritative
   departure events; reconnect validates and replays the gapless log in exact
   order; and missing timed-out orders never imply a transfer. Lobby browser,
