...
| M6 | Foundation started | Deterministic objective-aware planner and replay driver; recovered outer planner, all family dispatch values, action history, selector inventory and strategic routing; exact pre-city six-player reaction initialization, directional attitude matrix, resolver-boundary recovery, combat/Control reactions, hostility-aware attack candidates, canonical hashing and v6 save/v5 replay persistence; identified separate 0/1/2 per-computer resolution band by mentality | Resolve mode-6/pair-ratio hostility guards and modes 10-16 command guards; map every resolution-band action formula; replace provisional scalar scoring with handler-exact policy; capture reference boundaries and larger-player/objective-completion stress cases; M5 gate |
...
Current automated baseline: the solution builds successfully, 345 tests
