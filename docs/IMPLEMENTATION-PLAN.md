...
- The current implementation baseline includes the recovered sector Combat +
  ...
  passes 1,343 tests. Native saves are v20, replay is v22, and the canonical
  hash is v23.
...
| M5 | Foundation started | Scenario predicates, exact all-scenario scores and competition standings with player-slot tie order/trailing inactive players, binary-ordered end-turn evaluation and global sole-survivor completion; manual-backed Siege setup marks all six starting controlled HQ sectors important and the city renders paired gray pylons; Big Man accrual, Eliminate cleanup, state-derived winners, all five award projections, outcome events/notifications and hashing; recreation-native v20 snapshots and v22 authoritative-operation replays with bounded validation, complete immutable event-history authentication with append-cached canonical bytes, four-target multi-item command persistence, read-back-before-promotion, last-valid-generation backups, client recovery flows, and reusable migration machinery; complete one-to-three-item Give/Sell selection and resolution workflows | Exact Siege pylon art capture, special objective/tie fixtures, additional persistence fuzz/soak coverage, remaining management UI deliverable and M4 gate |
...
Current automated baseline: the solution builds successfully, 1343 tests
