...
Current automated baseline: the solution builds successfully, 1505 tests
...
2. Harden the modern online path with automatic host-snapshot recovery where
   the protocol already permits it, and
   a deterministic recorded policy for departed seats before implementing AI
   takeover. Replay v26 now provides a one-way, clean-boundary, hash-authenticated
   Human-to-Computer transfer and verifies lockstep AI planning after it. The
   remaining wire step must identify a real departure (not merely a missing
   timed-out order) and replay its exact event-log position during reconnect.
   Lobby browser, spectator, and chat work remain separate product
