...
- Maintain the bounded decoder that converts the user-owned `HELP/Chaos.hlp`
  ...
  representation with readable text, titles, contents hierarchy, and listed
  status. Recover links, inline formatting, and context IDs where possible,
  without checking in or distributing the copyrighted help text.
...
- Maintain the F1 Help route and cross-platform in-game viewer backed by the
  ...
  clear legal-copy/import error when help content is unavailable. Extend the
  current screen-based contextual topic mapping as original context IDs are
  recovered, and restore inline topic links and rich formatting.
...
| M7 | Foundation started | Original city ownership layers and site/gang portraits are rendered; Item Information and completed-Research reports play the dedicated 15-frame `PX04xxx` item rotations, and management-panel artwork uses measured template apertures; Detailed Combat synchronizes equipped, unarmed, retaliation and detected-police sounds to their corresponding animation clips; all nine recovered general-effect slots load and proven setup selection/rejection cues play; recovered title/setup Track 2, gameplay Tracks 3-8, and endgame Track 9 programs repeat with focus pause/resume; Options persists base/current statistics, automatic Detailed/Simple combat, bounded panel motion, independent 0-10 audio, idle-gang warnings, the planning timer, and recreation-native windowed/borderless-fullscreen mode; global F11 display switching preserves the 640x460 virtual coordinate system; recovered Current/Detailed-on/Slide-on/idle-warning-on defaults are used, panel entrances follow the recovered horizontal 344-pixel primary travel and quarter-second benchmark target, and legacy color depth stays explicitly on; bounded local WinHelp decoding and a cross-platform F1 viewer navigate the original 59 contents-listed topics from 80 decoded records, with screen-specific entry and safe fallback | Complete atlas/event integration, animations, help links/formatting/numeric context IDs, remaining sound-effect triggers, validate original Options close/adjacent-panel/combat cadence and music menu/persistence boundaries, validate timer/native playback, video, golden screens and M1-M6 dependencies |
...
Current automated baseline: the solution builds successfully, 1175 tests
