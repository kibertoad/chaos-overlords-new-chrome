...
- Static-analysis conclusions, addresses, confidence, and rejected hypotheses
  belong in the `original-internals/` research log; player-visible intended
  behavior belongs in `AI-SPEC.md`; implementation status and the next proof
  gate belong in `PARITY-MATRIX.md`. Never commit proprietary assets or decompiled original
  source text.
...
The generated finding, rule, and decision indexes and tables of contents are
rebuilt with `node tools/update-doc-indexes.mjs` (`--check` reports stale
blocks); `ASSET-CATALOG.md` is regenerated by the extractor's `--catalog` mode.

### 4.1 Documents to maintain

| Document | Required content | Completion gate |
|---|---|---|
| `README.md` (in `docs/`) | Catalog of every document by purpose, topic index from subsystem to rule IDs, finding IDs, and parity rows, identifier and status conventions, canonical identities | Every document and every finding/rule family is reachable from the index |
...
| `ORIGINAL-INTERNALS.md` and the `original-internals/` documents it indexes | Recovered modules, global state, turn phases, arrays, limits, object relationships, state mutation order, RNG and timing | Enough structure to explain every parity fixture; every finding reachable from the index |
