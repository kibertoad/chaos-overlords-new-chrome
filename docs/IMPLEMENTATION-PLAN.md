...
| M6 | Foundation started | Deterministic non-mutating objective-aware command planner, cash budgeting, validated hire choice, per-player Human/Computer setup and replay-recorded client driver; global four-level AI Mentality with docs-backed fair-play and aggression direction; static identification of the original zero-based mentality global, query selector and first threshold consumers; timed scenarios complete and objective scenarios run 20-turn deterministic replay-verified two-AI tournaments | Classify mentality write sites and map all difficulty branch outcomes/weights and visibility; reference decision snapshots; larger-player and objective-completion tournaments; M5 gate |
...
| M8 | Foundation started | Windows local launcher; legal-copy extraction; self-contained Windows package and GOG-aware Inno installer with visible stages, retryable source selection, nonzero failure exit and runtime error dialog; Linux amd64 `.deb`; macOS arm64/x64 application-bundle `.pkg`; manually dispatched validation and four-installer GitHub Release workflows; pull-request/manual zizmor gate; clean-room run `34403047147` passed Windows, Linux, both macOS architectures, and all installer jobs; zizmor run `34403047115` passed | Signing/notarization, native interactive tests, accuracy audit, compatibility and full release gate |
