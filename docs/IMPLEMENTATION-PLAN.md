...
- The current implementation baseline includes the recovered sector Combat +
  ...
  12-16 sector-selection kernels, live recovered hire-placement rules, and 965
...
- Preserve the recovered streamed programs: Track 2 repeats for title/setup,
  ...
  Preserve the recovered 0-10 volume scale and level-5 default. Implement its
  Options control, determine preference persistence and precise menu restart
  boundaries, and keep music independently adjustable. Missing or unreadable
  music must degrade silently to gameplay without music, never stall the turn
  loop.
...
| M7 | Foundation started | Original city ownership layers and site/gang portraits are rendered; resolved equipped attacks route item-defined original weapon sounds; recovered title/setup Track 2, gameplay Tracks 3-8, and endgame Track 9 programs repeat with focus pause/resume, use the exact level-5 default/0-10 conversion, and degrade safely when absent or unreadable | Complete atlas/event integration, animations, modern viewer for locally extracted legacy help, remaining sound effects, implement the soundtrack Options control, recover preference persistence and exact menu restart boundaries, validate native playback, video, golden screens and M1-M6 dependencies |
...
Current automated baseline: the solution builds successfully, 965 tests
...
playback cannot start. Remaining trigger and animation work, soundtrack
Options control, preference persistence and exact menu restart boundaries,
native validation, video, and exact visual behavior are still required.
...
6. Complete setup alignment, remaining management hit maps, atlas semantics,
   ...
   sound triggers, the soundtrack Options control, preference persistence and
   exact menu restart boundaries plus native playback validation, video triggers, and
