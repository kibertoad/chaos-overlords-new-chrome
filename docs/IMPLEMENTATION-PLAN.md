...
- Chaos uses generated sector Income + Force + Chaos for each gang, earns cash
  ...
  storage, and may trigger a crackdown. The recreation currently substitutes
  recomputed owner Cash for Income and persists a synthetic sector-Chaos value.
...
| M3 | Foundation started | Ordered base Upkeep economy; non-mutating original City/Sector Financial projection for pending contracts, upkeep, queued cash actions, income and Chaos; all Instant and Transaction actions with binary-recovered player/roster-slot order, deferred Give application and Sell payout overwrite; separate Terminate-before-Move player/roster passes with retained last-gang sector ownership; cumulative per-gang Influence, same-item Research completion suppression, and board-ordered grouped Control; phase-opening effective-stat snapshots for simultaneous Instant rolls; one phase-opening owner/defense snapshot and binary-recovered player-slot tie selection with a leading neutral candidate at zero margin for cross-player Control conflicts, including execution-time Crackdown lockout; per-turn Chaos with binary player/roster roll order, pre-Combat Crackdown creation, persisted prepared outcomes, and post-Transaction grouped payout; same-turn new-police combat/duration tick; pre-planning activation of completed-site stats; manual-backed sector/site ownership invariants for all three special buildings; Research caps; binary-recovered Factory division/rounding with delayed acquisition/replacement coverage; recovered 0/1/2 difficulty pools and thresholds for Heal, Influence, Research, Chaos, plus high-band owned-sector Crackdown reduction | Correct the generated-Income/owner-Cash substitution, remove synthetic sector Chaos, restore the owner-only Cash display, correct `SMGISLANDS`, then complete binary police notification fixtures, transaction runtime corroboration, and the M2 gate |
...
only the displayed record through save v23, replay v26, and canonical hash v26;
