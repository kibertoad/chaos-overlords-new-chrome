...
| M5 | Foundation started | Scenario predicates, exact all-scenario scores and competition standings with player-slot tie order/trailing inactive players, binary-ordered end-turn evaluation and global sole-survivor completion; manual-backed Siege setup marks all six starting controlled HQ sectors important and the city renders paired gray pylons; Big Man accrual, Eliminate cleanup, state-derived winners, all five award projections, outcome events/notifications and hashing; recreation-native v21 snapshots and v23 authoritative-operation replays with bounded validation, complete immutable event-history and phase-boundary-history authentication with append-cached canonical event bytes plus legacy shape/sequence validation, four-target multi-item command persistence, read-back-before-promotion, last-valid-generation backups, client recovery flows, and reusable migration machinery; complete one-to-three-item Give/Sell selection and resolution workflows | Exact Siege pylon art capture, special objective/tie fixtures, additional persistence fuzz/soak coverage, remaining management UI deliverable and M4 gate |
