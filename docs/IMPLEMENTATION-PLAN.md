...
| M3 | Foundation started | Ordered base Upkeep economy; non-mutating original City/Sector Financial projection for pending contracts, upkeep, queued cash actions, income and Chaos; all Instant and Transaction actions with binary-recovered player/roster-slot order, deferred Give application and Sell payout overwrite; Move/Terminate; grouped Influence and Control; phase-opening effective-stat snapshots for simultaneous Instant rolls; one phase-opening owner/defense snapshot and binary-recovered player-slot tie selection with a leading neutral candidate at zero margin for cross-player Control conflicts, including execution-time Crackdown lockout; per-turn Chaos; Crackdown lifecycle; influenced-site stats; manual-backed sector/site ownership invariants for all three special buildings; Research caps; binary-recovered Factory division/rounding with same-turn acquisition/replacement coverage; recovered 0/1/2 difficulty pools and thresholds for Heal, Influence, Research, Chaos, plus high-band owned-sector Crackdown reduction | Binary police/call-order fixtures, transaction runtime corroboration, abandoned-sector behavior, and M2 gate |
...
Current automated baseline: the solution builds successfully, 1137 tests
