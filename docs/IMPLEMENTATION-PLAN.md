...
- The current implementation baseline includes the recovered sector Combat +
  ...
  12-16 sector-selection kernels, live recovered hire-placement rules, and 1,093
  passing tests. Native saves are v19, replay is v20, and the canonical hash is
  v22.
...
| M5 | Foundation started | Scenario predicates, exact all-scenario scores and competition standings with player-slot tie order/trailing inactive players, binary-ordered end-turn evaluation and global sole-survivor completion; manual-backed Siege setup marks all six starting controlled HQ sectors important and the city renders paired gray pylons; Big Man accrual, Eliminate cleanup, state-derived winners, all five award projections, outcome events/notifications and hashing; recreation-native v16 snapshots and v17 authoritative-operation replays with bounded validation, read-back-before-promotion, last-valid-generation backups, client recovery flows, and reusable migration machinery; equipment Give recipient workflow | Exact Siege pylon art capture, special objective/tie fixtures, additional persistence fuzz/soak coverage, remaining management UI deliverable and M4 gate |
