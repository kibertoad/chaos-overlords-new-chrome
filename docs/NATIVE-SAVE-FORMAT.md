...
Status: implemented format version 11
...
- UTF-8 JSON with camel-case property names and `formatVersion: 11`.
...
| Member | Contents |
|---|---|
| `formatVersion` | Schema discriminator; currently `11` |
...
| `runtime` | Phase coordinator, RNG state/count, fixed-six-player AI reactions/directional attitudes, six current and previous AI hire roles, six-by-81 AI family records, six encoded hire-placement anchors, command queue/counter, event history/counter, notification queues/counters, phase hashes, and outcome |
...
Readers accept versions 1 through 9. Older documents migrate formerly implicit
...
recreation slots use the inactive-sector encoding. The appropriate legacy canonical hash is verified
before the migrated state is returned. Unknown
...
single-action validation. Replay version 11 embeds native version 10, and
version 12 embeds native version 11. Version 11 uses the version-13 hash and
derives placement anchors; version 10 uses the version-12 hash and migrates the new modifier to false; version 9
uses its version-11 hash, versions 7 and 8
...
canonical state hash is version 14. Initial-state migration is covered for
