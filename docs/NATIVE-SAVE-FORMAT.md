...
Status: implemented format version 10
...
- UTF-8 JSON with camel-case property names and `formatVersion: 10`.
...
| Member | Contents |
|---|---|
| `formatVersion` | Schema discriminator; currently `10` |
...
the player's name. The appropriate legacy canonical hash is verified
...
10 uses the version-12 hash and migrates the new modifier to false; version 9
...
canonical state hash is version 13. Initial-state migration is covered for
