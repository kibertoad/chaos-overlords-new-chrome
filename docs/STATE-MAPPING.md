...
Verdict: every field is `same` or `representation`, so this row can be `complete`. What
`InfluencedBy` changes is covered under `tolerance` and `cash_yield` in FMT-STATE-002.
