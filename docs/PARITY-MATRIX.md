...
| City/Sector UI | Original screen/panel workflow | Authoritative-state client now uses recovered density/site generation, fixed HQ candidates, Right Hands setup and deferred original-range hire offers; keyboard board/phase actions remain provisional | Static executable evidence High for generator; prototype layout/interaction Low | UI atlas, setup screens, screen router, mouse input, seed/context fixture and golden screens |
