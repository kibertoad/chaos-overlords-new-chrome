...
| Audio | 28 WAV resources and triggers | All WAVs extracted; resolved equipped-weapon attacks retain and route the event-time item Sound index to `SND00500`-`SND00518` without replaying historical load/replay events or losing the cue when elimination clears equipment. The recovered nine-slot general table loads `SND00200`-`SND00208` with its intentional slot-5 gap; setup selection/rejection uses proven slots 3/4, the idle confirmation uses panel slots 0/1, and the planning countdown uses warning slots 7/8. Effects have an independent persisted 0-10 level, default 6, with the exact original amplitude conversion. | Static loader, Options, setup, modal, and timer call graphs plus file/table inspection High; remaining trigger/overlap mapping Medium-Low | Map bare-hand, command, remaining UI, police, impact and notification cues; validate overlap/interruption |
