...
| Police | Combat 20, Detect 12, stealth detection curve; 3–5-turn presence extended by another Crackdown; third occurrence within five turns neutralizes control; Control prohibited while present | Deterministic duration/extension and two-entry occurrence history with v4 save migration, control/influence cleanup, displaced-owner notification, and visible countdown; provisional phase-wide attacks, detection, damage, events, notifications, casualties, and submission/runtime Control lockout implemented | Manual/save layout High for constants/curve/duration/history/lockout; duration RNG, defense formula, message wording and ordering Low | Binary fixtures for timing, RNG consumption, duration draw/extension and three-in-five boundary/notification |
...
| Native saves | Versioned safe recreation format | Version 4 snapshots preserve explicit sector income, Crackdown duration/history, and all authoritative runtime state; v1-v3 migration, deterministic encoding, 16 MiB bound, atomic replacement, backup recovery, F5/F9 quick saves and end-turn autosave implemented | Recreation format; deterministic continuation and migration tests, High | Add checked-in versioned fixtures |
| Replays | Deterministic recreation playback | Version 2 records a v4 initial snapshot plus every public match mutation, validation result and resulting state hash; bounded loader rejects divergence and out-of-band recording mutations; full Crackdown trigger/countdown/police-combat replay and atomic client F6/F10 record/playback flow implemented | Recreation format; full-turn deterministic and file-store tests, High | Add checked-in fixture/migration and animated playback controls |
| City/Sector UI | Original screen/panel workflow | Routed title/setup/handoff/city/sector/gang/finance/ranking/items/Give/combat-summary/search/commands/hire/events/endgame flow starts authoritative recovered cities; the city composites each sector from original owner-colored layers and overlays original police art for active Crackdowns; sector/hire/gang/Give views use original portrait sheets; finance/ranking use authoritative projections; items queue validated Research/Equip/Give/Sell with legal recipient selection; Combat Summary presents relevant attack/police results; Search respects cooperative detection; all mutations are replay-recorded | Static executable evidence High for generator and mapped atlas identities; interaction/coordinates Medium-Low | Complete UI atlas/setup detail, remaining original management panels, seed/context fixture and golden screens |
