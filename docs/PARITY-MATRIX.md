...
| Options | Persistent audiovisual and presentation choices | The recreation uses the recovered defaults: Current gang statistics (`prefsBaseStats=0`), Detailed Combat on (`prefsCombat=1`), Slide Panels on (`prefsSlide=1`), and Warn If Idle Gangs on (`prefsFreeGang=1`); version-4 preferences preserve existing choices and acquire the three presentation defaults added in version 5. Panel entrances follow the recovered downward-to-upward 344-pixel travel and quarter-second benchmark target rather than the earlier provisional horizontal motion. | Initialized bytes, exact preference-loader mapping, direct consumer references, and panel copy/calibration functions High for defaults/direction/primary travel/target duration; alternate-panel classification and runtime cadence Medium-Low | Capture open/close panel cadence, classify the 320-pixel alternate, and recover original preference/menu restart boundaries |
...
| Combat animation | Item-selected attack/hit sequences, retaliation, evasion and police attacks | All 98 `PX07xxx` strips decoded as eight 64x64 frames; event-time weapons route normal and mirrored attack/hit pairs into `PX05014`, including question/evasion and police-car/beam extras; the Detailed preference controls automatic playback, while Combat Summary can replay either mode manually; Escape/Cancel clears the bounded queue without exposing or mutating simulation phases | Asset dimensions/frame inspection, item table and original Combat capture High; exact cadence/color key Medium | Reproduce the original Detailed freeze; validate cadence, compositing/color key and bare-hand style selection against executable captures |
