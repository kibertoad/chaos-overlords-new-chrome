...
| Notifications | Bounded per-player ordered queues | Mechanical notification schema plus recovered 16-entry per-player capacity and drop-oldest overflow order implemented | Static report-recorder table stride/count/copy/cursor path High; kind filtering and delivery timing Medium-Low | Map report-kind filtering and exact handoff/viewer cursor transitions |
