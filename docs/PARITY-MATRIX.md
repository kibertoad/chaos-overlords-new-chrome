...
| City/Sector UI | Original screen/panel workflow | Title/setup/handoff/city/commands/events/endgame router starts authoritative recovered cities; deterministic Core projection exposes all validator-approved gang/sector/site/item commands; keyboard/mouse phase actions and replay-recorded notification dismissal are implemented provisionally | Static executable evidence High for generator; prototype layout/interaction Low | Complete UI atlas/setup detail, richer target workflow, remaining original panels, seed/context fixture and golden screens |
