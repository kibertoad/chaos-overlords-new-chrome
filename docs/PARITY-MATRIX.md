...
| Music | Original CD-audio soundtrack, represented by eight GOG Ogg tracks (`Track02`-`Track09`) | Runtime streams every available extracted track in physical CD order and wraps after `Track09`; missing or unreadable music disables itself without affecting gameplay | GOG pack, High format/source identity; provisional playback order, original selection policy unknown | Recover track mapping, sequencing, transitions, repeat/advance, and preference/volume behavior; validate native playback on Windows, Linux, and macOS |
