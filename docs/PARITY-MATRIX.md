...
| Sector control | Grouped Force + Control minus income, defenders and site Support; zero margin has a 50% chance; overthrow resets influence | Grouped resolver, deterministic recorded zero-margin draw, ownership/site reset, Support and Overthrow accounting implemented | Manual equation/zero chance High; simultaneous ordering Low | Cross-player tie/conflict, income-definition and crackdown fixtures |
...
| Chaos | Force + Chaos + sector Income; controlled/uncontrolled payout; sector-wide crackdown suppression; negative effective tolerance auto-busts | Phase-wide grouped resolver, cash/statistics, persisted Chaos, commandless negative-tolerance crackdown state and broadcast notifications implemented | Manual base formula/auto-bust High; pooling Medium; accumulation/rounding/order Low | Controlled binary experiments at tolerance boundaries and police aftermath |
