...
| Options | Persistent audiovisual and presentation choices | The recreation uses the recovered defaults: Current gang statistics (`prefsBaseStats=0`), Detailed Combat on (`prefsCombat=1`), Slide Panels on (`prefsSlide=1`), and Warn If Idle Gangs on (`prefsFreeGang=1`); version-4 preferences preserve existing choices and acquire the three presentation defaults added in version 5 | Initialized bytes, exact preference-loader mapping, and direct consumer references High; exact panel/combat cadence Medium | Capture panel/combat cadence and original preference/menu restart boundaries |
