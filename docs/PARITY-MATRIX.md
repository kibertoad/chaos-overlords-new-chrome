...
| City/Sector UI | Original screen/panel workflow | Title/setup/handoff/city/commands/hire/events/endgame router starts authoritative recovered cities; Core projects validator-approved commands; Hire exposes three offers, explicit placement and snubbing; keyboard/mouse phase actions and notification dismissal are replay-recorded | Static executable evidence High for generator; prototype layout/interaction Low | Complete UI atlas/setup detail, richer target workflow, remaining original panels, seed/context fixture and golden screens |
