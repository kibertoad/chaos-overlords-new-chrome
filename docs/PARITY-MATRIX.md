...
| Music | Original CD-audio soundtrack, represented by eight GOG Ogg tracks (`Track02`-`Track09`) | Recovered screen programs are wired: title/setup repeats Track 2, gameplay advances Tracks 3-8 then repeats, and endgame repeats Track 9; losing focus pauses and regaining focus resumes; the recovered level-5 default and exact 0-10 volume conversion are applied; missing or unreadable music disables itself without affecting gameplay | GOG pack and static MCI/aux-volume call graph, High for track ranges/order/repeat/focus/default-volume behavior; exact menu restart and preference-persistence boundaries Medium | Implement the Options control, capture menu restart and preference-persistence boundaries, and validate native playback on Windows, Linux, and macOS |
