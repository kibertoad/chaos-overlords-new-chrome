...
| Native saves | Versioned safe recreation format | Version 1 stream/file snapshots preserve authoritative state, phase/RNG, commands, events, notifications, hashes and outcomes; deterministic encoding, 16 MiB bound, atomic replacement, backup recovery and client F5/F9 flow implemented | Recreation format; deterministic continuation tests, High | Add v1 fixture/migration harness and autosave policy |
| Replays | Deterministic recreation playback | Version 1 records an initial snapshot plus every public match mutation, validation result and resulting state hash; bounded loader rejects divergence and out-of-band recording mutations | Recreation format; full-turn deterministic test, High | Add checked-in v1 fixture/migration and client record/playback flow |
