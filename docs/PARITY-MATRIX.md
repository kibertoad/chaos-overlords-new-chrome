...
| Audio | 28 WAV resources and triggers | All WAVs extracted; resolved equipped-weapon attacks retain and route the event-time item Sound index, unarmed attacks route ordinary/Martial-Arts sounds 0/1, detected police route sound 18, and evasion remains silent. Historical load/replay events do not replay cues, and elimination cannot erase the event-time equipped sound. The recovered nine-slot general table loads `SND00200`-`SND00208` with its intentional slot-5 gap; setup selection/rejection uses proven slots 3/4, the idle confirmation uses panel slots 0/1, pending Last Turn Events alert with slot 6, and the planning countdown uses warning slots 7/8. Slot 9 is loaded but has no gated-wrapper call site in the supported executable. Effects have an independent persisted 0-10 level, default 6, with the exact original amplitude conversion. | Static loader, Detailed Combat, Options, setup, report-recorder, modal, and timer call graphs plus file/table inspection High; remaining trigger/overlap mapping Medium-Low | Sequence retaliation audio with its second clip; map remaining command, UI, and impact cues; validate slot-6 suppression and overlap/interruption |
