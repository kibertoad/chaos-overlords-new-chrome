...
| Notifications | Bounded per-player ordered queues | Presentation-independent mechanical notification schema plus a filtered Last Turn Events projection | Manual event list High; current kind filtering, capacity, and delivery timing Medium-Low | Validate report-kind filtering, capacity, and exact handoff/viewer cursor transitions |
| Comlink | View/send messages among human Overlords; newest 16 received messages retained | Authoritative active-human send validation, deterministic multi-recipient delivery, exact bounded inboxes, unread state, v17 persistence, v20 hashing, and v18 replay implemented; client panels pending | Manual workflow and static 166-byte queue/count/copy/cursor paths High; exact record fields Medium | Implement `PX05018` composition/recipient controls and Comlink View with unread indication |
...
| Objectives | Ten named scenarios and objective thresholds | Provisional live end-of-turn evaluation; fresh Siege matches designate all six starting controlled HQ sectors and render the manual-described paired gray pylons; Big Man center-sector point accrual, Eliminate cleanup, authoritative projection, tied winners, events/notifications and outcome hashing | Manual pages 12–15, High for thresholds/weights/effects and Siege mapping; timing, exact pylon artwork, and special edges Low | Binary boundary/tie/ordering fixtures and exact Siege marker capture |
...
| Options | Persistent audiovisual and presentation choices | The recreation uses the recovered defaults: Current gang statistics (`prefsBaseStats=0`), Detailed Combat on (`prefsCombat=1`), Slide Panels on (`prefsSlide=1`), and Warn If Idle Gangs on (`prefsFreeGang=1`); version-4 preferences preserve existing choices and acquire the three presentation defaults added in version 5. Panel entrances follow the recovered right-to-left 344-pixel primary travel and quarter-second benchmark target. | Initialized bytes, exact preference-loader mapping, rectangle/blit field order, direct consumer references, and panel copy/calibration functions High for defaults/direction/primary travel/target duration; adjacent-buffer panel classification and runtime cadence Medium-Low | Capture open/close panel cadence, identify the 320-pixel adjacent-buffer form, and recover original preference/menu restart boundaries |
...
| Input | Original mouse plus modern keyboard | Letterboxed virtual-coordinate mouse hit testing and keyboard navigation implemented; the recovered Add/Remove/Begin/Cancel setup rectangles retain their pressed identity, draw the exact `PX00140` held-inside tile, restore the baked control outside, act only on release inside the same control, and cancel release outside; city sectors select on single-click and open detail on bounded same-sector double-click; Hire portraits drag as scaled tokens through controlled-sector validation from both city and detailed-sector modes, with a detailed-workspace drop targeting its centered sector; detailed gang portraits use movement-threshold disambiguation so stationary double-click opens details while drag/drop queues only validator-legal neighboring Move targets; Done presents the recovered optional confirmation when an active gang has no queued command | Current client and original Hire/sector captures, Medium-High; executable scan and Help High for setup buttons and idle-gang warning | Remaining original hit maps, right-click/cancel and configurable bindings |
| Help | Original WinHelp topics and contextual Help command | The supported local `Chaos.hlp`/`CHAOS.CNT` pair is decoded during legal-copy import into a versioned 80-topic document; F1 opens a bounded cross-platform topic viewer with contextual entry, full topic reachability, keyboard/mouse paging, and safe unavailable-content handling. Its Attack topic displays the corrected Force-inclusive, simultaneous rule instead of leaving players with the manual's omission. | Exact legal-copy decode independently matched at 57,640 normalized source characters/SHA-256; High for readable text and topic order, Low for omitted inline formatting/link/context metadata | Recover links, formatting, context IDs, and reference-match contextual entry points |
| Audio | 28 WAV resources and triggers | All WAVs extracted; resolved equipped-weapon attacks retain and route the event-time item Sound index, unarmed attacks route ordinary/Martial-Arts sounds 0/1, detected police route sound 18, and evasion remains silent. Detailed playback emits each cue at its clip's first animation tick, so retaliation waits for the reversed second clip. Historical load/replay events do not replay cues, and elimination cannot erase the event-time equipped sound. The recovered nine-slot general table loads `SND00200`-`SND00208` with its intentional slot-5 gap; every routed panel transition and the idle confirmation use entry/exit slots 0/1 only while Slide Panels is enabled, full local-setup push buttons use slot 2, setup selection/rejection uses slots 3/4, pending Last Turn Events alert uses slot 6, and the planning countdown uses warning slots 7/8. Slot 9 is loaded but has no gated-wrapper call site in the supported executable. Effects have an independent persisted 0-10 level, default 6, with the exact original amplitude conversion. | Static loader, panel-helper/caller set, button helpers, Detailed Combat, Options, setup, report-recorder, modal, and timer call graphs plus file/table inspection High; remaining trigger/overlap mapping Medium-Low | Map remaining command, UI, and impact cues; validate slot-6 suppression and overlap/interruption |
| Combat animation | Item-selected attack/hit sequences, retaliation, evasion and police attacks | All 98 `PX07xxx` strips decoded as eight 64x64 frames; event-time weapons route normal and mirrored attack/hit pairs into `PX05014`, including question/evasion and police-car/beam extras; the Detailed preference controls automatic playback, while Combat Summary can replay either mode manually; Escape/Cancel clears the bounded queue without exposing or mutating simulation phases. Deterministic twin-resolution coverage proves fully drained Detailed playback leaves the same combat rolls, Force, phase hash, and complete authoritative hash as Simple presentation. | Asset dimensions/frame inspection, item table and original Combat capture High; presentation/state separation tests High; exact cadence/color key Medium | Reproduce the original Detailed freeze; validate cadence, compositing/color key and bare-hand style selection against executable captures |
