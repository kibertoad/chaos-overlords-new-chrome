...
public sealed partial class ChaosGame
{
    ...
    private bool DrawSeparatedSlidingPanel(Viewport viewport, int slideOffset)
    {
        if ((_screens.Current is not (ClientScreen.Hire or ClientScreen.Ranking
                or ClientScreen.CombatSummary or ClientScreen.SectorGangs)) || _state is null
            || _batch is null || _pixel is null || _font is null)
            return false;
        ...
        _batch.Draw(_pixel, new Rectangle(0, 0, 640, 460), new Color(8, 10, 12));
        if (_screens.Current == ClientScreen.SectorGangs
            && _sectorGangReturnScreen == ClientScreen.Sector)
            DrawSectorDetails(_batch, _pixel, _font, _state);
        else if ((_screens.Current is ClientScreen.Ranking or ClientScreen.CombatSummary)
        ...
        switch (_screens.Current)
        {
            ...
            case ClientScreen.SectorGangs:
                DrawSectorGangsPanel(_batch, _pixel, _font, _state);
                break;
        }
        _batch.End();
