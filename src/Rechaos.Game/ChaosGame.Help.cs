...
public static class HelpContentAugmentation
{
    ...
    [
        ...
        ("CHAOS", "Chaos", "Each human gang separately rolls max(0, sector Income + Force + effective Chaos) dice at 5+. "
            ...
            + "The first and second Crackdown within five turns do nothing more. The third makes the sector neutral, clears its sites' progress and brings police for 3 to 5 turns. "
            + "Chaos is paid after the transactions: a player's successes in one sector are combined, sectors the player controls at that moment pay the full total and other sectors pay trunc(total / 2). "
            + "A gang killed in that turn's combat is not paid. Police already present from earlier turns do not stop the pay."),
        ("CONTROL", "Control", "Control uses no dice. Each player totals Force + effective Control from its gangs ordered to Control the sector. A sector with police in it is not settled. "
            + "A challenger's margin is its total minus the sector's Income and Support, where Support is the completed sites' Support set before planning. The owner's margin is its own total plus Force + Control of each of its active gangs in the sector that is not hiding. "
            + "The largest positive margin takes the sector, and players tied for it are drawn between at random. When the best margin is 0 a no-change candidate joins the draw, so a lone zero-margin challenger wins half the time. "
            + "Only players with a Control order in the sector, and its owner, take part in the comparison. "
            + "A takeover clears the sites' progress, but the sector's Cash was set before planning, so the next Upkeep still pays the new owner the tax plus the old sites' Cash once."),
        ...
        ("CONTHIRE", "Hire", "Selecting an offer reserves the recruit; it does not spend cash immediately. After every command phase has resolved at the end of the turn, Hire checks the resulting cash balance and charges the displayed contract cost. Sales and Chaos proceeds can therefore fund the contract. If that balance cannot cover the cost, the reservation fails without a charge or a new gang, and Last Turn Events explains the failure. HIRE SHORTFALL shows only the projected amount missing at this Hire check. After a successful contract, the recruit is active. The following Upkeep charges every active gang, including that recruit. Normal play crosses this boundary automatically, so the contract and first upkeep can appear together when control returns. Upkeep may make cash negative; it does not remove the gang."),
        ...
        ("SNITCH", "Snitch", "Snitch subtracts 3 from the sector's base Tolerance, free and whatever the player's cash. After all Instant commands every base below 1 is raised to 1 and every base above 40 lowered to 40. The Tolerance the Chaos test reads is the base plus the completed sites' Tolerance, set before planning, so a Snitch first counts in the next turn's test."),
        ("TIS", "The Inner Sanctum", "The cashflow figure is a projection of queued income and spending, new contracts, and the next Gang Upkeep. It is not money already received or spent. Paid commands are checked when their phase resolves: Bribe costs $3, and Equip uses the item's price after any Factory discount. If the player lacks enough cash at that exact point, the command fails without a charge. Hire is checked later, after all command-resolution phases. Upkeep is mandatory and may take cash below zero; active gangs remain employed, but later Bribe, Equip, and Hire attempts fail whenever their own resolution check cannot be afforded."),
