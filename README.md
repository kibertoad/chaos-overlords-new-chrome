...
| Combat and police | Simultaneous gang combat, retaliation, evasion, detection, casualties, equipment loss, Crackdowns, police attacks, original combat/result panels with fitted sector apertures, decoded animations and cadence, and equipped, unarmed, and detected-police sound cues. Detailed Combat controls automatic bounded animation playback without changing resolution; Simple Combat leaves the non-animated summaries available without blocking input. Selected summary results can be replayed in Detail mode and cancelled immediately. | Color-key details, bare-hand presentation, and several reveal/police ordering edges remain to validate. |
