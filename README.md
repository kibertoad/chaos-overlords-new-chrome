...
| Online play | The title screen can host or join private, code-based matches through the central service at `chaos-overlords.dinorefurb.com` or a custom self-hosted coordination service. Host/Join is selected explicitly, so the guest-only join-code field is disabled while hosting and gains a clipboard Paste button while joining; a lobby's code can be copied to the system clipboard. The host uploads a verified rolling server autosave after every confirmed turn. The game retains the last membership locally, flags an uncleanly interrupted match on the next launch, and can reconnect the same host or guest seat. The service reconstructs that session from the newest verified snapshot plus later sealed turns, restores its current draft, and resumes the ordered event stream. See [Multiplayer](docs/MULTIPLAYER.md). This does not reproduce the original network protocols. |
