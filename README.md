...
| Online experience | Departed human seats transfer to deterministic AI control at the authoritative event-log position, including gaplessly validated reconnect reconstruction; active humans who merely time out remain human. There is no public lobby browser, spectator/join-in-progress flow, lobby chat, or online Comlink integration. A desync still depends on the host supplying a snapshot. Security and deployment limitations are documented in [Multiplayer](docs/MULTIPLAYER.md). |
