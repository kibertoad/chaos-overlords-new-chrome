...
purpose and maps each game subsystem to its `RULE-*` IDs in `GAME-RULES.md`,
its `BIN-*` finding IDs in `ORIGINAL-INTERNALS.md`, and its parity rows. Read it
before searching the documents by hand, and keep it current when a document is
added, renamed, or changes purpose.

Record new evidence where its family lives: static executable findings in
`ORIGINAL-INTERNALS.md` under a new unique `BIN-<AREA>-<NNN>` heading, intended
mechanics in `GAME-RULES.md` under a `RULE-<AREA>-<NNN>` heading, deliberate
deviations as a dated entry at the top of `DECISIONS.md`, and status changes in
`PARITY-MATRIX.md`. Finding and rule IDs are stable handles referenced from
...
blocks without writing. `docs/ASSET-CATALOG.md` is generated by
