#!/usr/bin/env node
// Checks spec/, PARITY.md and DEVIATIONS.md against version 1 of the documentation standard
// (https://dinorefurb.com/documentation-standard/#checks), and writes the four indexes in
// spec/index/.
//
// Usage:
//   node tools/check-spec.mjs            check, then rewrite stale indexes
...
for (let k = 0; k < argv.length; k++) {
  const known = ["--check", "--no-ksy", "--base", "--glossary"];
  if (argv[k].startsWith("-") && !known.includes(argv[k])) {
    console.error(`unknown option ${argv[k]}; see --help`);
    process.exit(2);
  }
  if (argv[k] === "--base" || argv[k] === "--glossary") k++;
}
const checkOnly = argv.includes("--check");
const skipKsy = argv.includes("--no-ksy");
...
if (!existsSync(specDir)) {
  console.error("No spec/ directory.");
  process.exit(1);
}
...
for (let k = 0; k < argv.length; k++) if (argv[k] === "--glossary" && argv[k + 1]) {
  const extra = readFileSync(argv[k + 1], "utf8").replace(/\r\n/g, "\n");
...
// directory, or a pattern whose last part holds a placeholder such as nn or xxx, is left alone.
{
  ...
  const checkPaths = (file, text) => {
    for (const m of text.matchAll(/\b(?:DATA|HELP|MUSIC)\/[A-Za-z0-9_./-]*[A-Za-z0-9]/g)) {
      const p = m[0];
...
if (!skipKsy) {
  ...
  if (compiler && ksys.length) {
    try {
      const out = join(repoDir, "artifacts", "ksy-check");
...
const header = "<!-- Generated by tools/check-spec.mjs. Do not edit. -->\n\n";
...
for (const [name, content] of Object.entries(indexes)) {
  ...
  if (checkOnly) problem(p, "is stale; run node tools/check-spec.mjs");
