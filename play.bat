...
if exist "%ASSET_DIR%\manifest.json" (
  call dotnet run --project src\Rechaos.Extractor -- --verify-output --output "%ASSET_DIR%" --quick >nul 2>nul
  if not errorlevel 1 set "ASSETS_READY=1"
)
...
if "%ASSETS_READY%"=="0" (
  ...
  call dotnet run --project src\Rechaos.Extractor -- --source "%GAME_SOURCE%" --output "%ASSET_DIR%"
  if errorlevel 1 (
    popd
    exit /b 1
  )
