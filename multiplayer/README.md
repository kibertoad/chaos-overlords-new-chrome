...
| `RATE_LIMIT_PER_MINUTE` | `120` | Unauthenticated calls (handshake, browse, create, join) per client address per minute. Every per-minute budget is at least `1`: `0` is refused at startup rather than admitting one call a minute. |
