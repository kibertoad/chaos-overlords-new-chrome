...
export function createSqliteStorage(db: SqliteDatabase): MultiplayerStorage {
  return {
    matches: sqliteMatchRepository(db),
  ...
  return {
    ...
    async claimLateJoinOrder(matchId) {
      ...
      return takeJoinOrder(
        ...
        sql`max(${matches.joinCounter}, ${nextFree}) + 1`,
      )
    },
    ...
    async updateRuntimeGameSettings(matchId, gameSettings, updatedAt) {
      const rows = await db
        .update(matches)
        .set({
          settings: sql`json_set(${matches.settings}, '$.gameSettings', json(${JSON.stringify(gameSettings)}))`,
          updatedAt,
        })
...
function sqliteSnapshotRepository(db: SqliteDatabase): SnapshotRepository {
